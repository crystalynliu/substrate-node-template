...
#[test]
fn create_claim_failed_when_claim_is_oversize() {
	new_test_ext().execute_with( || {
		let claim: Vec<u8> = vec![1; 51];
...
#[test]
fn revoke_claim_failed_when_claim_do_not_exist() {
	new_test_ext().execute_with( || {
		let claim: Vec<u8> = vec![0, 1];
...
#[test]
fn transfer_claim_failed_when_claim_do_not_exist() {
	new_test_ext().execute_with( || {
		let claim: Vec<u8> = vec![0, 1];
...
#[test]
fn transfer_claim_failed_when_origin_is_not_claim_owner() {
	new_test_ext().execute_with( || {
		let claim: Vec<u8> = vec![0, 1];
		let _ = PoeModule::create_claim(Origin::signed(1), claim.clone());
